...
export const cats = sqliteTable(
  "cats",
  {
    ...
    // Saúde
    fiv: text("fiv", { enum: ["positive", "negative", "not_tested"] }).notNull(),
    felv: text("felv", { enum: ["positive", "negative", "not_tested"] }).notNull(),
    castrated: integer("castrated", { mode: "boolean" }).notNull().default(false),
